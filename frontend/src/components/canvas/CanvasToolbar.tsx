...
import { Plus, Edit2, Trash2, ZoomIn, ZoomOut, Maximize2, User, Package, MapPin, Film, Sparkles } from 'lucide-react';
...
export function CanvasToolbar({ projectId, onZoomIn, onZoomOut, onZoomReset, visibleTypes, onToggleType }: CanvasToolbarProps) {
        ...
        {canvasList.map(canvas => (
          ...
          >
            {editingCanvasId === canvas.canvas_id ? (
              ...
              <>
                <span
                  onClick={() => setActiveCanvas(canvas.canvas_id)}
                  className="text-sm"
                >
                  {canvas.name}
                </span>
