          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-400">
                ...
                {canViewHistoricalFailedRefunds && <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-red-400" />历史失败待退费</span>}
              </div>
            </div>
                    ...
                    <div className="relative h-[180px] border-l border-b border-gray-600/80">
                      ...
                      <div className="absolute inset-x-1 bottom-0 top-0 flex items-end gap-[2px]">
                        {chartItems.map(item => {
                          ...
                          const displayedVideoCost = canViewHistoricalFailedRefunds ? Math.max(item.video_cost - item.failed_video_cost, 0) : item.video_cost;
                          const videoHeight = item.total_cost > 0 ? (displayedVideoCost / item.total_cost) * 100 : 0;
                          const failedVideoHeight = canViewHistoricalFailedRefunds && item.total_cost > 0 ? (item.failed_video_cost / item.total_cost) * 100 : 0;
                          return (
                            <div key={item.key} className="flex-1 min-w-0 h-full flex flex-col justify-end group">
                              <div
                                ...
                                style={{ height: `${height}%` }}
                                title={canViewHistoricalFailedRefunds
                              ...
                              >
                                ...
                                {canViewHistoricalFailedRefunds && item.failed_video_cost > 0 && <div className="bg-red-400" style={{ height: `${failedVideoHeight}%` }} />}
                              </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {participantRows.map(user => (
                        <tr key={user.username} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                          ...
                          <td className="text-right py-2 pr-4 text-green-400">{fmt(canViewHistoricalFailedRefunds ? Math.max((user.video_cost || 0) - (user.failed_video_cost || 0), 0) : (user.video_cost || 0))}</td>
                          {canViewHistoricalFailedRefunds && <td className="text-right py-2 pr-4 text-red-400">{fmt(user.failed_video_cost || 0)}</td>}
                    ...
                    <tfoot>
                      <tr className="text-gray-300 font-semibold">
                        ...
                        <td className="text-right py-2 pr-4 text-green-400">{fmt(participantRows.reduce((s, u) => s + (u.video_cost || 0) + (canViewHistoricalFailedRefunds ? 0 : (u.failed_video_cost || 0)), 0))}</td>
                        {canViewHistoricalFailedRefunds && <td className="text-right py-2 pr-4 text-red-400">{fmt(participantRows.reduce((s, u) => s + (u.failed_video_cost || 0), 0))}</td>}
