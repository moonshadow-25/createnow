...
def delete_user(user_id: str) -> bool:
    """软删除用户（is_active=False），不允许删除唯一的 admin"""
    data = _read_users()
    # 找到目标用户
    target = None
    for user in data.get("users", []):
...
def ensure_default_admin() -> None:
    """启动时调用：若 users.json 不存在或无任何用户，创建默认账号"""
    data = _read_users()
    if data.get("users"):
        return  # 已有用户，不重复创建

    default_username = "menglaoshi"
    default_password = "menglaoshi123"
    ...
    print("=" * 60)
    print("[INFO] 已创建默认管理员账号：")
    print(f"       用户名：{default_username}")
    print(f"       密  码：{default_password}")
    print("       请登录后尽快修改密码！")
    print("=" * 60)
