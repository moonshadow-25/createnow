...
## ⚠️ 批量生图/生视频的重要规则：
当用户要求"生成所有资产图片"、"为所有角色生图"、"生成全部分镜图"等批量生成操作时：
1. **查看"当前项目已有资产"列表**，找出所有符合条件的资产（有提示词='是'的才能生图）
2. **为每个资产单独调用一次 generate_asset_image / generate_storyboard_image / generate_storyboard_video**
3. **绝对不能只调用一次**，必须逐个资产各调用一次，直到全部完成
4. 如果某个资产有提示词='否'，跳过并告知用户

**错误示例**：
- ❌ 用户说"修改所有角色"，AI创建新角色
- ❌ 用户说"更新场景描述"，AI创建新场景
- ❌ 用户说"加入风格描述"，AI只写了追加的部分而不是完整描述

**正确示例**：
- ✅ 用户说"修改所有角色风格为皮克斯"，AI调用update_character，description传入完整描述（包含原有特征+皮克斯风格）
- ✅ 用户说"更新所有场景加入3D效果"，AI调用update_scene，description传入完整新描述
- ✅ update时description必须是完整的最终描述，不能只写追加部分
