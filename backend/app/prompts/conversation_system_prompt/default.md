...
4. **无时标对白拆分公式**：当无时标且对白为主时，按“目标60字≈15秒”估算分镜数，`N=ceil(对白总字数/60)`；例如对白约240字，至少拆4段。
5. **均衡分配规则**：按标点优先切分后，对 `N` 段做均衡分配，尽量使每段对白接近平均值，避免前几段过满、最后一段过短；确有余数时，允许最后一段相对更短。
6. **对白硬约束**：单分镜对白总字数（含台词、旁白、OS）上限90字；超过90字时必须按标点优先截断并续拆到后续分镜，拆分后需重新标注说话人和对白类型。
...
5. **创建新分镜**（批量生成时）：
   ...
   - 先规划所有分镜：
     ...
     - 若无时标且对白为主，最少分镜数按 `ceil(对白总字数/60)` 计算
     - 按标点进行均衡切分，尽量让每镜对白量接近平均值
   - 对白字数规则：每镜建议控制在60字左右，硬上限90字，超过必须继续拆分
