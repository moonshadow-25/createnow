...
2. **description 只能写剧本原文片段（若原文片段前有场次行，则首行保留原样；否则不要添加首行），禁止改写、总结、润色**
...
description 字段必须包含该分镜对应的原始剧本片段原文（不可改写或总结），供后续修改时参考。若原文片段前本来就有场次行，就把该场次行原文放在首行；如果没有场次行，就不要额外添加首行，直接粘贴原文片段。
...
5. **创建新分镜**（批量生成时）：
   ...
   - description 必须是该分镜对应的原始剧本片段原文；若片段前本来就有场次行，首行保留场次行原文，否则不要额外添加首行
   - 先规划所有分镜：
